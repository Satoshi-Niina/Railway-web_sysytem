...
export function OperationPlanningChart() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        maintenancePlans = await apiCall<any[]>(`monthly-maintenance-plans?month=${currentMonth}`)
      } catch (error) {
  ...
  const handleSavePlan = async (planData: Partial<OperationPlan>) => {
    try {
      if (editingPlan) {
        // 既存計画の更新
        const updatedPlan = await apiCall(`operation-plans/${editingPlan.id}`, {
          method: 'PUT',
          body: JSON.stringify({
        ...
        const createdPlan = await apiCall('operation-plans', {
          method: 'POST',
  ...
  const handleDeletePlan = async (planId: number) => {
    try {
      await apiCall(`operation-plans/${planId}`, {
        method: 'DELETE',
      })
  ...
  const exportToCSV = async () => {
    try {
      ...
      const fetchUrl = `${API_BASE_URL}/operation-plans?${params.toString()}`
